...
def get_supertonic_assets_path() -> Path:
    """
    Downloads the supertonic whole repo with onnx models
    """
    local_dir = snapshot_download(
        repo_id=SUPERTONIC_REPO_ID,
        # optional: choose your own folder instead of HF cache:
        local_dir="./supertonic",
    )
    return Path(local_dir)

SUPERTONIC_REPO_ID = "Supertone/supertonic"

console = Console()

class SupertonicTTS:
    def __init__(self, **kwargs):
        self.denoising_steps = 5 # number of denoising steps
        whole_repo_dir = get_supertonic_assets_path()
        onnx_dir =  f"{whole_repo_dir}/onnx" # path to the onnx models directory
        voice_style_paths = [f"{whole_repo_dir}/voice_styles/M1.json"] # Voice style file path(s). Can specify multiple files for batch processing
        use_gpu = False # Use GPU for inference or CPU - set false for now as gpu mode not available
        ...
        # --- 3. Load Voice Style --- #
        self.style = load_voice_style(voice_style_paths, verbose=False)
        self.bsz = len(voice_style_paths)
    ...
    def generate_audio(self, text, output_path="output.wav"):
        ...
        sample_rate=24000
        wav, duration = self.text_to_speech(text, self.style, self.denoising_steps)
        for b in range(self.bsz):
            w = wav[b, : int(sample_rate * duration[b].item())]  # [T_trim]
            sf.write(output_path, w, sample_rate)
